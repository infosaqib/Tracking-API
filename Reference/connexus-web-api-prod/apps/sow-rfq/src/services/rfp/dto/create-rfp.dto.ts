import { ApiProperty } from '@nestjs/swagger';
import { RfpPortfolioType, RfpStatusType, SowThemeTypes } from '@prisma/client';
import { Type } from 'class-transformer';
import {
  IsArray,
  IsDateString,
  IsEnum,
  IsNotEmpty,
  IsOptional,
  IsString,
  ValidateNested,
} from 'class-validator';

export class RfpDocumentDto {
  @ApiProperty({
    description: 'Name of the document',
    example: 'attachment.pdf',
  })
  @IsString()
  @IsNotEmpty()
  fileName: string;

  @ApiProperty({
    description: 'Path of the document',
    example: 'attachments/attachment.pdf',
  })
  @IsString()
  @IsNotEmpty()
  filePath: string;

  @ApiProperty({
    description: 'Type of the document',
    example: 'application/pdf',
    required: false,
  })
  @IsString()
  @IsOptional()
  fileType?: string;

  @ApiProperty({
    description: 'Size of the document in bytes',
    example: 1024,
    required: false,
  })
  @IsOptional()
  fileSizeBytes?: bigint;

  @ApiProperty({
    description:
      'SHA256 or similar hash of the document (used for duplicate detection)',
    example: '3a7bd3e2360a3d80a5c5b1f6c3f8a7c2b4d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1',
    required: false,
  })
  @IsString()
  @IsOptional()
  fileHash?: string;
}

class RfpPropertyAttachmentFileDto {
  @ApiProperty({ description: 'Name of the attachment', example: 'photo.jpg' })
  @IsString()
  @IsNotEmpty()
  fileName: string;

  @ApiProperty({
    description: 'Path of the attachment',
    example: 'attachments/photo.jpg',
  })
  @IsString()
  @IsNotEmpty()
  filePath: string;

  @ApiProperty({
    description: 'Type of the attachment',
    example: 'image/jpeg',
    required: false,
  })
  @IsString()
  @IsOptional()
  fileType?: string;

  @ApiProperty({
    description: 'Size of the attachment in bytes',
    example: 2048,
    required: false,
  })
  @IsOptional()
  fileSizeBytes?: number;

  @ApiProperty({
    description:
      'SHA256 or similar hash of the attachment (used for duplicate detection)',
    example: '3a7bd3e2360a3d80a5c5b1f6c3f8a7c2b4d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1',
    required: false,
  })
  @IsString()
  @IsOptional()
  fileHash?: string;
}

class RfpPropertyAttachmentDto {
  @ApiProperty({
    description: 'Property ID for the attachment',
    example: 'd9e8f7g6-h5i4-3j2k-1l0m-n9o8p7q6r5s4',
  })
  @IsString()
  @IsNotEmpty()
  propertyId: string;

  @ApiProperty({
    description: 'List of attachments for this property',
    type: [RfpPropertyAttachmentFileDto],
  })
  @IsArray()
  @ValidateNested({ each: true })
  @Type(() => RfpPropertyAttachmentFileDto)
  attachments: RfpPropertyAttachmentFileDto[];
}

export class CreateRfpDto {
  @ApiProperty({
    description: 'Name of the RFP',
    example: 'Annual Landscaping RFP',
  })
  @IsString()
  @IsNotEmpty()
  rfpName: string;

  @ApiProperty({
    description: 'Description of the RFP',
    example: 'Annual landscaping services for all properties',
    required: false,
  })
  @IsString()
  @IsOptional()
  description?: string;

  @ApiProperty({
    description: 'ID of the client',
    example: 'c8a9d7e6-f5b4-4a3e-2b1c-9d8e7f6a5b4c',
  })
  @IsString()
  clientId: string;

  @ApiProperty({
    description: 'ID of the RFP service',
    example: 'a1b2c3d4-e5f6-7g8h-9i0j-k1l2m3n4o5p6',
  })
  @IsString()
  serviceId: string;

  @ApiProperty({
    enum: RfpPortfolioType,
    description: 'Portfolio type of the RFP',
    example: RfpPortfolioType.SINGLE_PROPERTY,
  })
  @IsEnum(RfpPortfolioType)
  portfolioType: RfpPortfolioType;

  @ApiProperty({
    enum: SowThemeTypes,
    description: 'Theme type for the RFP',
    example: SowThemeTypes.DEFAULT,
    required: false,
  })
  @IsEnum(SowThemeTypes)
  @IsOptional()
  themeType?: SowThemeTypes;

  @ApiProperty({
    description: 'ID of the contract',
    example: 'b2c3d4e5-f6g7-8h9i-0j1k-l2m3n4o5p6q7',
    required: false,
  })
  @IsString()
  @IsOptional()
  contractId?: string;

  @ApiProperty({
    description: 'RFP due date',
    example: '2025-12-31T23:59:59.999Z',
    required: false,
  })
  @IsDateString()
  @IsOptional()
  rfpDueDate?: string;

  @ApiProperty({
    description: 'RFP award date',
    example: '2026-01-31T23:59:59.999Z',
    required: false,
  })
  @IsDateString()
  @IsOptional()
  rfpAwardDate?: Date;

  @ApiProperty({
    description: 'RFQ date',
    example: '2025-11-30T23:59:59.999Z',
    required: false,
  })
  @IsDateString()
  @IsOptional()
  rfqDate?: Date;

  @ApiProperty({
    description: 'RFI date',
    example: '2025-10-01T00:00:00.000Z',
    required: false,
  })
  @IsDateString()
  @IsOptional()
  rfiDate?: Date;

  @ApiProperty({
    description: 'Contract start date',
    example: '2025-10-15T00:00:00.000Z',
    required: false,
  })
  @IsDateString()
  @IsOptional()
  contractStartDate?: Date;

  @ApiProperty({
    description: 'Term of contract',
    example: '12 months',
    required: false,
  })
  @IsString()
  @IsOptional()
  termOfContract?: string;

  @ApiProperty({
    description: 'Status of the RFP',
    enum: RfpStatusType,
    required: false,
  })
  @IsEnum(RfpStatusType)
  @IsOptional()
  status?: RfpStatusType;

  @ApiProperty({
    description: 'List of property IDs associated with the RFP',
    type: [String],
    example: ['d9e8f7g6-h5i4-3j2k-1l0m-n9o8p7q6r5s4'],
  })
  @IsArray()
  @IsString({ each: true })
  propertyIds: string[];

  @ApiProperty({
    description: 'List of scope of work IDs associated with the RFP',
    type: [String],
    example: ['e1f2g3h4-i5j6-7k8l-9m0n-o1p2q3r4s5t6'],
  })
  @IsArray()
  @IsString({ each: true })
  scopeOfWorkIds: string[];

  @ApiProperty({
    description: 'List of documents to be attached to the RFP',
    type: [RfpDocumentDto],
    required: false,
  })
  @IsOptional()
  @IsArray()
  @ValidateNested({ each: true })
  @Type(() => RfpDocumentDto)
  documents?: RfpDocumentDto[];

  @ApiProperty({
    description: 'List of property attachments grouped by property',
    type: [RfpPropertyAttachmentDto],
    required: false,
  })
  @IsOptional()
  @IsArray()
  @ValidateNested({ each: true })
  @Type(() => RfpPropertyAttachmentDto)
  rfpPropertyAttachments?: RfpPropertyAttachmentDto[];

  @ApiProperty({
    description: 'RFP extended due date',
    example: '2025-12-31T23:59:59.999Z',
    required: false,
  })
  @IsDateString()
  @IsOptional()
  rfpExtendedDueDate?: string;
}
